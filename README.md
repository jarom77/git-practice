...
How can you get the changes to `README.md` and your Python on the same branch though? We need to perform a *merge* using a *pull request*. Click on "Pull Requests" in the top bar, and click the green button "New Pull Request". If a "base repository" is listed, change it to your own fork (the one with your username). When do we a merge, we're merging the changes *from* the branch on the right *to* the branch on the left, following the arrow. So "main" should be on the left and your new branch on the right. Add a title for the changes, and list any pertinent information in the description, such as which files were changed, what was changed, and why. It's also helpful to include what kind of testing you did to ensure your code works. Create the pull request.
